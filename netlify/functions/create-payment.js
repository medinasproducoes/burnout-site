...
exports.handler = async (event) => {
  ...
  try {
    const SITE_URL = 'https://medinasproducoes-site.netlify.app';

    // Gera um order_nsu único para rastrear esta compra específica
    const orderNsu = `burnout-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
      ...
      items: [
        {
          ...
          description: 'Burnout em Silencio - Ebook'
        }
      ],
      order_nsu: orderNsu,
    ...
    // A API pode retornar o link em campos diferentes — checamos as possibilidades
